...
describe.only("map-registry", () => {
  ...
  describe("Create Maps", () => {
    ...
    it("Creates a user map successfully", async () => {
      ...
      assertEqual(metadata.mapId, mapId1, "Map ID should match");
      assertEqual(metadata.name, mapName1, "Map name should match");
      assertEqual(metadata.description, mapDescription1, "Map description should match");
      assertEqual(metadata.creator.toString(), user.toString(), "Creator should be the user");
      assertEqual(metadata.isDefault, false, "Should be a user map");
      assertTrue(metadata.createdAt.gt(new anchor.BN(0)), "Created at timestamp should be set");
      assertEqual(metadata.createdAt.toString(), metadata.updatedAt.toString(), `Created ${metadata.createdAt} and updated timestamps ${metadata.updatedAt} should match initially`);
      ...
      const registry = await program.account.mapRegistry.fetch(mapRegistryPDA);
      assertEqual(registry.userMapsCount, 1, "User maps count should be 1");
      assertEqual(registry.defaultMapsCount, 0, "Default maps count should still be 0");

      // Fetch and verify user map index
      const userIndex = await program.account.userMapIndex.fetch(userMapIndexPDA);
      assertEqual(userIndex.mapCount, 1, "User should have 1 map");
      assertEqual(userIndex.mapIds[0], mapId1, "User's map list should contain the map ID");
    });
    ...
    it("Creates a default map successfully", async () => {
      ...
      assertEqual(metadata.isDefault, true, "Should be a default map");

      // Fetch and verify registry counters
      const registry = await program.account.mapRegistry.fetch(mapRegistryPDA);
      assertEqual(registry.defaultMapsCount, 1, "Default maps count should be 1");
      assertEqual(registry.userMapsCount, 1, "User maps count should still be 1");
    });
    ...
    it("Fails to create a map with data exceeding maximum size", async () => {
      ...
      try {
        ...
        assertFail("Should have thrown an error for data too large");
      } catch (error) {
        const errorStr = error.toString().toLowerCase();    

        
        // Check for either the custom error or the serialization error
        if (!errorStr.includes("mapdatatoolarge") &&
            !errorStr.includes("map data exceeds") &&
            !errorStr.includes("encoding overruns") &&
            !errorStr.includes("too large")) {
          throw new Error(`Should throw MapDataTooLarge error, got: ${error.toString()}`);
        }
        console.log("✓ Correctly rejected oversized map data");
      }
    });
  });
  ...
  describe("Update Map Metadata", () => {
    ...
    it("Updates map name successfully", async () => {
      ...
      assertEqual(metadataAfter.name, newName, "Name should be updated");
      assertEqual(metadataAfter.description, originalDescription, "Description should remain unchanged");
      assertTrue(
    ...
    it("Updates map description successfully", async () => {
      ...
      assertEqual(metadata.description, newDescription, "Description should be updated");
    });
    ...
    it("Updates both name and description successfully", async () => {
      ...
      const metadata = await program.account.mapMetadata.fetch(mapMetadataPDA);
      assertEqual(metadata.name, newName, "Name should be updated");
      assertEqual(metadata.description, newDescription, "Description should be updated");
    });
    ...
    it("Fails to update metadata if not the creator", async () => {
      ...
      try {
        await program.methods
          ...
          .accounts({
            mapMetadata: mapMetadataPDA,
            user: otherUser.publicKey, // Explicitly set the user account
          })
          .signers([otherUser])
          .rpc();

        assertFail("Should have thrown an unauthorized error");
      } catch (error) {
        const errorStr = error.toString().toLowerCase();
        // Check for unauthorized or constraint error
        if (!errorStr.includes("unauthorized") && !errorStr.includes("constraint")) {
          throw new Error(`Should throw Unauthorized error, got: ${error.toString()}`);
        }
  ...
  describe("Update Map Data", () => {
    ...
    it("Fails to update data exceeding maximum size", async () => {
      ...
      try {
        ...
        const errorStr = error.toString().toLowerCase();
        // Check for either the custom error, serialization error, or encoding error
        if (!errorStr.includes("mapdatatoolarge") &&
            !errorStr.includes("map data exceeds") &&
            !errorStr.includes("encoding overruns") &&
            !errorStr.includes("too large")) {
  ...
  describe("Delete Map", () => {
    ...
    it("Deletes a map successfully", async () => {
      ...
      try {
        ...
        assertFail("Map metadata account should be closed");
      } catch (error) {
        assertInclude(error.toString(), "Account does not exist");
      }

      try {
        await program.account.mapData.fetch(mapDataPDA);
        assertFail("Map data account should be closed");
      } catch (error) {
        assertInclude(error.toString(), "Account does not exist");
      }
  ...
  describe("Multiple Maps Management", () => {
    it("Creates and manages multiple maps for a single user", async () => {
      ...
      for (const { mapId, mapMetadataPDA } of mapPDAs) {
        const metadata = await program.account.mapMetadata.fetch(mapMetadataPDA);
        assertEqual(metadata.mapId, mapId, "Map ID should match");
      }
